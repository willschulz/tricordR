...
server <- function(input, output) {
  ...
  network_data_prepped_d3 <- reactive({invalidateLater(refresh_time)
    ...
    prep_network_data_d3_spirals(study_name,participant_group,assignment_group, include_protected = input$include_protected, compliance_threshold_daycount = 1/24, anonymize_participants = input$anonymize_participants)#note: including protected creates a huge data lift in the long term... figure out how to mitigate
  })
